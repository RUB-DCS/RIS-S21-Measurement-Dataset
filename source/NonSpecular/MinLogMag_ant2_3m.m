...
%  Ant1 distance 1-7m, Ant2 distance 3m
subplot 221
hold on
for i=12:17
    load(fullfile(folderPath,sprintf("%02i.mat",i)))
    p(i-11)=plot(freqPoints/1e9,results.algoSEmin{256}.logmagDataPoints);
end
datatip(p(1),'DataIndex',76); % Datatip at 5.375GHz dist 1m
legend('1m','1,5m','2m','3m','5m','7m')
title('Min SE; Ant1 1-7m 0°; Ant2 3m 45°')
...
for i=12:17
    load(fullfile(folderPath,sprintf("%02i.mat",i)))
    p(i-11)=plot(freqPoints/1e9,results.algoGreedyMin{100}.logmagDataPoints);
end
datatip(p(1),'DataIndex',76); % Datatip at 5.375GHz dist 1m
legend('1m','1,5m','2m','3m','5m','7m')
title('Min Greedy; Ant1 1-7m 0°; Ant2 3m 45°')
xlabel('Frequency [GHz]')
ylabel('S21 [dB]')
ylim([-70 -10])
grid on
...
for i=12:17
    ...
    p(i-11)=plot(freqPoints/1e9,S21.logmagDataPoints); %Plot handles
end
datatip(p(1),'DataIndex',76); % Datatip at 5.375GHz dist 1m
legend('1m','1,5m','2m','3m','5m','7m')
title('Min Geometric; Ant1 1-7m 0°; Ant2 3m 45°')
xlabel('Frequency [GHz]')
ylabel('S21 [dB]')
ylim([-90 -10])
grid on
...
for i=12:17
    load(fullfile(folderPath,sprintf("%02i.mat",i)))   
    p(i-11)=plot(freqPoints/1e9,reference.RISallOff.logmagDataPoints); 
end
datatip(p(1),'DataIndex',76); % Datatip at 5.375GHz dist 1m
legend('1m','1,5m','2m','3m','5m','7m')
title('RIS all elements off; Ant1 1-7m 0°; Ant2 3m 45°')
xlabel('Frequency [GHz]')
ylabel('S21 [dB]')
ylim([-70 -10])
grid on
